...
def filter_by_hobby(people_list: list, hobby: str) -> list:
    ...
    """
    new_people_list = []
    new_people_list = filter(lambda human: hobby in human.hobbies, people_list)
    return list(new_people_list)
...
def sort_by_most_hobbies(people_list: list) -> list:
    ...
    """
    new_people_list = sorted(people_list, key=lambda human: (len(human.hobbies), human.last_name), reverse=True)
    return list(new_people_list)
...
def sort_people_and_hobbies(people_list: list) -> list:
    ...
    for human in people_list:
        human.hobbies = sorted(human.hobbies, key=lambda hobby: hobby)
    new_people_list = sorted(people_list, key=lambda human: human.full_name)
    return list(new_people_list)
