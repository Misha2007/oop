...
def read_from_file_into_list(filename: str) -> list:
    ...
    with open(filename) as file:
        ...
        if len(content) > 0:
            ...
            for i in content:
                cd = i.split(",")
                client = Client(cd[0], cd[1], int(cd[2]), int(cd[3]), int(cd[4]))
    ...
    with open(filename) as file:
        ...
        if len(content) > 0:
            ...
            for i in content:
                ...
                client = Client(cd[0], cd[1], int(cd[2]), int(cd[3]), int(cd[4]))
                list_of_clients.append(client)
            list_of_clients = filter(lambda client: client.bank == bank, list_of_clients)
    return list(list_of_clients)
    ...
    with open(filename) as file:
        ...
        if len(content) > 0:
            ...
            for i in content:
                ...
                client = Client(cd[0], cd[1], int(cd[2]), int(cd[3]), int(cd[4]))
                list_of_clients.append(client)
            sorted_list_of_clients = sorted(list_of_clients, key=lambda client: (client.earnings_per_day(), -client.account_age))
            if sorted_list_of_clients[-1].current_amount > sorted_list_of_clients[-1].starting_amount:
                return sorted_list_of_clients[-1]
            else:
                return None
    ...
    with open(filename) as file:
        ...
        if len(content) > 0:
            ...
            for i in content:
                ...
                list_of_clients.append(client)
            sorted_list_of_clients = sorted(list_of_clients, key=lambda client: (client.earnings_per_day(), client.account_age))
            if sorted_list_of_clients[0].current_amount < sorted_list_of_clients[0].starting_amount:
                return sorted_list_of_clients[0]
            else:
                return None
